...
def test_compiler_remove(mutable_config, mock_packages):
    ...
    assert spack.spec.Spec("gcc@=9.4.0") in spack.compilers.all_compilers()
    args = spack.util.pattern.Bunch(all=True, compiler_spec="gcc@9.4.0", add_paths=[], scope=None)
    spack.cmd.compiler.compiler_remove(args)
    assert spack.spec.Spec("gcc@=9.4.0") not in spack.compilers.all_compilers()
...
def test_compiler_find_prefer_no_suffix(no_compilers_yaml, working_env, compilers_dir):
    ...
    compilers = spack.compilers.all_compilers_from(no_compilers_yaml, scope="site")
